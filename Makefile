...
EQCHECK_TARGETS :=  $(LOCALMEM_TARGETS) #$(MICRO_TARGETS) $(TSVC_PRIOR_TARGETS) $(TSVC_NEW_TARGETS) $(LORE_MEM_TARGETS) $(LORE_NOMEM_TARGETS) $(PAPER_EX_TARGET) $(DIETLIBC_TARGET) $(OTHER_TARGETS) $(SPEC17_TARGETS) $(TSVC_PRIOR_LOCALS_TARGETS) $(TSVC_NEW_LOCALS_TARGETS) $(POPL_PAPER_EX_TARGETS) $(TSVC_POPL_TARGETS)
