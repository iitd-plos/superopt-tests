...
SPEC17_TARGETS := spec17
OTHER_TARGETS := bzip2 demo #reve ctests # soundness
EQCHECK_TARGETS := $(MICRO_TARGETS) #$(SPEC17_TARGETS) $(TSVC_PRIOR_TARGETS) $(TSVC_NEW_TARGETS) $(LORE_MEM_TARGETS) $(LORE_NOMEM_TARGETS) $(PAPER_EX_TARGET) $(DIETLIBC_TARGET) $(LOCALMEM_TARGETS) $(OTHER_TARGETS)
TARGETS :=  $(EQCHECK_TARGETS) #$(SPEC17_TARGETS) $(OOELALA_TARGETS) # $(CODEGEN_TARGETS)
