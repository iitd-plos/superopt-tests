...
cmd_name="${SUPEROPT_INSTALL_DIR}/bin/eq32 --enable-src-epsilon-paths --disable-dst-to-src-submap --disable-assumed-eqclasses --disable-houdini-axiom-based-timeout-dumps --smt-query-timeout=5"
...
speceq_debug() {
  ${cmd_name} --tmpdir-path=${spec_run_tmpdir} --proof="$1".proof --dyn-debug=invariants_dump,spec_coupling=3 --spec-iospecs="$1".iospecs "$1".spec "$1".c |& tee ${spec_run_out}
}
