...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_openbsd_m88k[VMEM_SIZE], final_mem_newlib_fast[VMEM_SIZE];
    ...
    const void *ret_newlib_fast = memcpy_newlib_fast(mem + dest_offset, mem + src_offset, n);
    memcpy(final_mem_newlib_fast, mem, VMEM_SIZE);

    if (ret_openbsd_m88k != ret_newlib_fast) {
        fprintf(stderr, "Fatal: found an inequivalence between the openbsd_m88k and newlib_fast implementations!\n");
        assert(0); 
    }
    if (!mem_eq(final_mem_openbsd_m88k, final_mem_newlib_fast, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the openbsd_m88k and newlib_fast implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_openbsd_m88k[MAX_SIZE], final_dest_newlib_fast[MAX_SIZE];
    ...
    const void *ret_newlib_fast = memcpy_newlib_fast(dest, src, n);
    ...
    if (!mem_eq(final_dest_openbsd_m88k, final_dest_newlib_fast, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the openbsd_m88k and newlib_fast implementations!\n");
        assert(0);
    }
    #endif
