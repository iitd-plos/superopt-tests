...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_dietlibc[VMEM_SIZE], final_mem_klibc[VMEM_SIZE];
    ...
    const void *ret_klibc = memccpy_klibc(mem + dest_offset, mem + src_offset, c, n);
    ...
    if (!mem_eq(final_mem_dietlibc, final_mem_klibc, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the dietlibc and klibc implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_dietlibc[MAX_SIZE], final_dest_klibc[MAX_SIZE];
    ...
    const void *ret_klibc = memccpy_klibc(dest, src, c, n);
    ...
    if (!mem_eq(final_dest_dietlibc, final_dest_klibc, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the dietlibc and klibc implementations!\n");
        assert(0);
    }
    #endif
