...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_dietlibc[VMEM_SIZE], final_mem_uClibc[VMEM_SIZE];
    ...
    if (!mem_eq(final_mem_dietlibc, final_mem_uClibc, VMEM_SIZE)) {
    ...
    char dest_initial[MAX_SIZE], final_dest_dietlibc[MAX_SIZE], final_dest_uClibc[MAX_SIZE];
    ...
    if (!mem_eq(final_dest_dietlibc, final_dest_uClibc, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the dietlibc and uClibc implementations!\n");
        assert(0);
    }
    #endif
