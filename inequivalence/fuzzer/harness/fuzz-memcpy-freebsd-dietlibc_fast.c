...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_freebsd[VMEM_SIZE], final_mem_dietlibc_fast[VMEM_SIZE];
    ...
    if (ret_freebsd != ret_dietlibc_fast) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd and dietlibc_fast implementations!\n");
        assert(0); 
    }
    if (!mem_eq(final_mem_freebsd, final_mem_dietlibc_fast, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd and dietlibc_fast implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_freebsd[MAX_SIZE], final_dest_dietlibc_fast[MAX_SIZE];
    ...
    if (!mem_eq(final_dest_freebsd, final_dest_dietlibc_fast, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd and dietlibc_fast implementations!\n");
        assert(0);
    }
    #endif
