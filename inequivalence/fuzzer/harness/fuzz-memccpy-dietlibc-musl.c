...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_dietlibc[VMEM_SIZE], final_mem_musl[VMEM_SIZE];
    ...
    const void *ret_musl = memccpy_musl(mem + dest_offset, mem + src_offset, c, n);
    ...
    if (!mem_eq(final_mem_dietlibc, final_mem_musl, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the dietlibc and musl implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_dietlibc[MAX_SIZE], final_dest_musl[MAX_SIZE];
    ...
    const void *ret_musl = memccpy_musl(dest, src, c, n);
    ...
    if (!mem_eq(final_dest_dietlibc, final_dest_musl, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the dietlibc and musl implementations!\n");
        assert(0);
    }
    #endif
