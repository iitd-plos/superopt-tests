...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_dietlibc[VMEM_SIZE], final_mem_netbsd_patched[VMEM_SIZE];
    ...
    if (!mem_eq(final_mem_dietlibc, final_mem_netbsd_patched, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the dietlibc and netbsd_patched implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_dietlibc[MAX_SIZE], final_dest_netbsd_patched[MAX_SIZE];
    ...
    if (!mem_eq(final_dest_dietlibc, final_dest_netbsd_patched, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the dietlibc and netbsd_patched implementations!\n");
        assert(0);
    }
    #endif
