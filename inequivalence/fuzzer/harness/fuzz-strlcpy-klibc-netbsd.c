...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_klibc[VMEM_SIZE], final_mem_netbsd[VMEM_SIZE];
    ...
    const size_t ret_netbsd = strlcpy_netbsd(mem + dest_offset, mem + src_offset, n);
    ...
    if (!mem_eq(final_mem_klibc, final_mem_netbsd, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the klibc and netbsd implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_klibc[MAX_SIZE], final_dest_netbsd[MAX_SIZE];
    ...
    const size_t ret_netbsd = strlcpy_netbsd(dest, src, n);
    ...
    if (!mem_eq(final_dest_klibc, final_dest_netbsd, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the klibc and netbsd implementations!\n");
        assert(0);
    }
    #endif
