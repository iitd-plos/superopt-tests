...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_freebsd_heimdal[VMEM_SIZE], final_mem_dietlibc[VMEM_SIZE];
    ...
    if (!mem_eq(final_mem_freebsd_heimdal, final_mem_dietlibc, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd_heimdal and dietlibc implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_freebsd_heimdal[MAX_SIZE], final_dest_dietlibc[MAX_SIZE];
    ...
    if (!mem_eq(final_dest_freebsd_heimdal, final_dest_dietlibc, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd_heimdal and dietlibc implementations!\n");
        assert(0);
    }
    #endif
