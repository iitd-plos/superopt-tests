...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_openbsd_m88k[VMEM_SIZE], final_mem_musl[VMEM_SIZE];
    ...
    if (ret_openbsd_m88k != ret_musl) {
        fprintf(stderr, "Fatal: found an inequivalence between the openbsd_m88k and musl implementations!\n");
        assert(0); 
    }
    if (!mem_eq(final_mem_openbsd_m88k, final_mem_musl, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the openbsd_m88k and musl implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_openbsd_m88k[MAX_SIZE], final_dest_musl[MAX_SIZE];
    ...
    if (!mem_eq(final_dest_openbsd_m88k, final_dest_musl, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the openbsd_m88k and musl implementations!\n");
        assert(0);
    }
    #endif
