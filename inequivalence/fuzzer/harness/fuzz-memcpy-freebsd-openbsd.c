...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_freebsd[VMEM_SIZE], final_mem_openbsd[VMEM_SIZE];
    ...
    if (ret_freebsd != ret_openbsd) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd and openbsd implementations!\n");
        assert(0); 
    }
    if (!mem_eq(final_mem_freebsd, final_mem_openbsd, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd and openbsd implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_freebsd[MAX_SIZE], final_dest_openbsd[MAX_SIZE];
    ...
    if (!mem_eq(final_dest_freebsd, final_dest_openbsd, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd and openbsd implementations!\n");
        assert(0);
    }
    #endif
