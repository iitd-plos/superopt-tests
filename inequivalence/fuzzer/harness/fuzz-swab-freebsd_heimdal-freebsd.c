...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_freebsd_heimdal[VMEM_SIZE], final_mem_freebsd[VMEM_SIZE];
    ...
    if (!mem_eq(final_mem_freebsd_heimdal, final_mem_freebsd, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd_heimdal and freebsd implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_freebsd_heimdal[MAX_SIZE], final_dest_freebsd[MAX_SIZE];
    ...
    if (!mem_eq(final_dest_freebsd_heimdal, final_dest_freebsd, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the freebsd_heimdal and freebsd implementations!\n");
        assert(0);
    }
    #endif
