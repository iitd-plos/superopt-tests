...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_musl_patched[VMEM_SIZE], final_mem_glibc[VMEM_SIZE];
    ...
    if (!mem_eq(final_mem_musl_patched, final_mem_glibc, VMEM_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the musl_patched and glibc implementations!\n");
        assert(0);
    }
    #else
    ...
    char dest_initial[MAX_SIZE], final_dest_musl_patched[MAX_SIZE], final_dest_glibc[MAX_SIZE];
    ...
    if (!mem_eq(final_dest_musl_patched, final_dest_glibc, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the musl_patched and glibc implementations!\n");
        assert(0);
    }
    #endif
