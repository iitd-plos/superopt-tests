...
int main(int argc, char** argv)
{
    ...
    char mem_initial[VMEM_SIZE], final_mem_glibc[VMEM_SIZE], final_mem_freebsd[VMEM_SIZE];
    ...
    if (!mem_eq(final_mem_glibc, final_mem_freebsd, VMEM_SIZE)) {
    ...
    char dest_initial[MAX_SIZE], final_dest_glibc[MAX_SIZE], final_dest_freebsd[MAX_SIZE];
    ...
    if (!mem_eq(final_dest_glibc, final_dest_freebsd, MAX_SIZE)) {
        fprintf(stderr, "Fatal: found an inequivalence between the glibc and freebsd implementations!\n");
        assert(0);
    }
    #endif
