...
/*    Metacharacters can be quoted with backslashes if QUOTE is nonzero.  *1/ */
...
/* /1* Prepend DIRNAME to each of N members of ARRAY, replacing ARRAY's */
/*    elements in place.  Return nonzero if out of memory, zero if successful. */
/*    A slash is inserted between DIRNAME and each elt of ARRAY, */
/*    unless DIRNAME is just "/".  Each old element of ARRAY is freed. */
/*    If ADD_SLASH is non-zero, allocate one character more than */
/*    necessary, so that a slash can be appended later.  *1/ */
...
/* /1* #if defined ENABLE_GLOB_BRACE_EXPANSION *1/ */
/* /1* /2* Find the end of the sub-pattern in a brace expression.  *2/ *1/ */
/* /1* const char * *1/ */
/* /1* __next_brace_sub (const char *cp, int flags) *1/ */
/* /1* { *1/ */
/* /1*   unsigned int depth = 0; *1/ */
/* /1*   while (*cp != '\0') *1/ */
/* /1*     if ((flags & GLOB_NOESCAPE) == 0 && *cp == '\\') *1/ */
/* /1*       { *1/ */
/* /1* 	if (*++cp == '\0') *1/ */
/* /1* 	  break; *1/ */
/* /1* 	++cp; *1/ */
...
/*       if (begin != NULL) */
/* 	{ */
/* 	  /1* Allocate working buffer large enough for our work.  Note that */
/* 	    we have at least an opening and closing brace.  *1/ */
/* 	  size_t firstc; */
/* 	  char *alt_start; */
/* 	  const char *p; */
/* 	  const char *next; */
...
/* 	  if (next == NULL) */
/* 	    { */
/* 	      /1* It is an illegal expression.  *1/ */
/* 	      return glob (pattern, flags & ~GLOB_BRACE, errfunc, pglob); */
/* 	    } */

/* 	  /1* Now find the end of the whole brace expression.  *1/ */
/* 	  rest = next; */
/* 	  while (*rest != '}') */
/* 	    { */
...
/* 		} */
/* 	    } */
/* 	  /1* Please note that we now can be sure the brace expression */
/* 	     is well-formed.  *1/ */
/* 	  rest_len = strlen (++rest) + 1; */

/* 	  /1* We have a brace expression.  BEGIN points to the opening {, */
...
/* 	    { */
/* 	      /1* This call is to set a new vector, so clear out the */
/* 		 vector so we can append to it.  *1/ */
/* 	      pglob->gl_pathc = 0; */
/* 	      pglob->gl_pathv = NULL; */
/* 	    } */
...
/* 	      /1* If we got an error, return it.  *1/ */
...
/* 	      if (*next == '}') */
/* 		/1* We saw the last entry.  *1/ */
/* 		break; */

/* 	      p = next + 1; */
/* 	      next = __next_brace_sub (p, flags); */
/* 	      /1* assert (next != NULL); *1/ */
/* 	    } */


/* 	  if (pglob->gl_pathc != firstc) */
/* 	    /1* We found some entries.  *1/ */
/* 	    return 0; */
