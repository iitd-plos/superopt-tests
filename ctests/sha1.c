...
int MYmymemcmp(const void *s1, const void *s2, size_t n);
...
{
  if (arch_big_endian) {
    MYmymemcpy(dst, src, numwords * sizeof(u32));
  } else {
    unsigned char * s, * d;
    unsigned char a, b;
...
int process_left_data(struct SHA1Context * ctx, unsigned char * data, unsigned long len)
{
  ...
  if (ctx->numbytes != 0) {
    t = 64 - ctx->numbytes;
    if (len < t) {
      MYmymemcpy(ctx->buffer + ctx->numbytes, data, len);
      ctx->numbytes += len;
      return 1;
    }
    MYmymemcpy(ctx->buffer + ctx->numbytes, data, t);
    SHA1_transform(ctx);
    data += t;
    len -= t;
  }
                   ...
                   unsigned long len)
{
  ...
  while (len >= 64) {
    MYmymemcpy(ctx->buffer, data, 64);
    SHA1_transform(ctx);
    data += 64;
    len -= 64;
  }
  /* Save remaining data */
  MYmymemcpy(ctx->buffer, data, len);
  ctx->numbytes = len;
}
...
void SHA1_finish(struct SHA1Context * ctx, unsigned char output[20])
{
  ...
  if (i > 56) {
    MYmymemset(ctx->buffer + i, 0, 64 - i);
    SHA1_transform(ctx);
    i = 0;
  }
  /* Pad to byte 56 with zeroes */
  MYmymemset(ctx->buffer + i, 0, 56 - i);
...
void do_test(unsigned char * txt, unsigned char * expected_output)
{
  ...
  SHA1_init(ctx);
  SHA1_add_data(ctx, txt, MYmystrlen((char *) txt));
  SHA1_finish(ctx, output);
  ok = MYmymemcmp(output, expected_output, 20) == 0;
  MYmyprintf("Test `%s': %s\n", 
...
{
  struct SHA1Context* ctx = MYmymalloc(sizeof(struct SHA1Context));
  int i;
...
int main(int argc, char ** argv)
{
  ...
  switch (u.b[0]) {
    ...
    default: MYmyprintf("Cannot determine endianness\n"); return 2;
  }
