...
__attribute__((noinline)) struct _play *make_play(int* all) /* make up the entire tree-like stuff */
{
  ...
  while (next_data(temp)) /* take next one, and break if none */
  {
    if (valid_data(temp)) /* if board position is possible */
    {
	    ...
	    int* unspec_tmp = copy_data(temp);
	    current -> next -> state = unspec_tmp;
	    struct _list* unspec_tmp2 = make_list(temp,val,all);
	    current -> next -> first = unspec_tmp2;
...
int main(void)
{
  ...
  int* all = MYmymalloc(sizeof(int));
  int* row = MYmymalloc(sizeof(int));
  int* col = MYmymalloc(sizeof(int));
...
#endif
  *all = 1;
  tree = make_play(all); /* create entire tree structure, not just the */
      ...
      if (temp != NULL)  /* temp = NULL when the poison pill is taken */
        {
          ...
          /* print it out nicely */
          MYmyprintf("player %d plays at (%d,%d)\n",player,*row,*col);
